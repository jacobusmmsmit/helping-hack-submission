...
```{r split data}
set.seed(42)
combi_filter <- combi %>% filter(!is.na(imd_2019))
...
```{r how many small entit belong to a bigger fra entity}
num_fra <- fra %>% select(LADCD, FRACD) %>% 
    group_by(FRACD) %>%
    count(LADCD) %>%
    select(FRACD, n) %>%
    unique()
...
bigset <- fra %>%
    ...
    left_join(num_fra, by = 'FRACD') %>%
    mutate(expenditure = expenditure / n) %>%
    select(-YEAR, -FRACD, -LADCD, -n)
    
new_ref <- bind_rows(bigset, ref)
```
...
sqrt(sum((predict.lm(mod4, newdata = filter(new_combi, !is.na(imd_2019))[-3])-filter(new_combi, !is.na(imd_2019))[3])^2)/311)
