...
```{r make predictions}
pred <- predict(mod2, newdata = filter(combi, is.na(imd_2019)))
predexist <- predict(mod2, newdata = filter(combi, !is.na(imd_2019)))
```
...
mod3 %>%
    ...
    select(.pred, imd_2015, lad_code, lad_name) %>%
    rename(imd_2019 = .pred) %>%
